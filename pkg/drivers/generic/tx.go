...
//var _ sqllog.Transaction = (*Tx)(nil)
...
//func (d *Generic) BeginTx(ctx context.Context, opts *sql.TxOptions) (sqllog.Transaction, error) {
...
func (t *Tx) GetCompactRevision(ctx context.Context) (int64, int64, error) {
	var compact, target sql.NullInt64
	row := t.queryRow(ctx, revisionIntervalSQL)

	if err := row.Scan(&compact, &target); err != nil {
